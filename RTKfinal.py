...
# API Laravel
LARAVEL_API_BASE_URL = "http://127.0.0.1:8088/api"
...
def detect_anomalies(prev_pos, curr_pos, prev_time, curr_time, prev_fix, curr_fix):
    ...
    if prev_fix in (4, 5) and curr_fix in (1, 2):
        alerts.append(f"RTK loss: {prev_fix} -> {curr_fix}")
    return alerts
...
def simulation_thread(pos_queue):
    ...
    while True:
        ...
        fix = 4 if random.random() > 0.1 else 1
        pos_queue.put((time.time(), {"lat": lat, "lon": lon, "fix_quality": fix}))
        time.sleep(1)
...
def hardware_thread(pos_queue):
    print("[GNSS] Opening serial port {F9P_NMEA_PORT}...")
    try:
